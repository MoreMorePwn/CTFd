import{$ as o,B as f,C as D,z as n}from"./main-CA4_OjGq.js";const i=window.POST_REVOKE_CALC||{};let p=null,u=null;const k={};function E(){const t=new URLSearchParams;i.bracketId&&t.set("bracket_id",i.bracketId),i.sort&&t.set("sort",i.sort);const e=t.toString();return e?`?${e}`:""}function g(t){return`/api/v1/post-revoke-calc${t}${E()}`}function r(t,e){o("#post-revoke-status").text(t||"").toggleClass("text-danger",!!e).toggleClass("text-muted",!e)}function d(t){if(!t||!t.errors)return"Post-Revoke Calc update failed.";const e=[];return Object.keys(t.errors).forEach(a=>{const s=t.errors[a];Array.isArray(s)?e.push(s.join(`
`)):e.push(String(s))}),e.join(`
`)||"Post-Revoke Calc update failed."}function $(t,e){return D.fetch(t,{credentials:"same-origin",headers:{Accept:"application/json","Content-Type":"application/json"},...e}).then(a=>a.json())}function C(t,e){return r("Saving..."),$(t,{method:"PATCH",body:JSON.stringify(e)}).then(a=>{if(!a.success)throw a;return r("Saved."),a.data})}function O(t){t.forEach(e=>{const a=o(`[data-account-row="${e.account_id}"]`);if(!a.length)return;a.find("[data-rank]").text(e.rank),a.find("[data-pre-score]").text(e.pre_score_display),a.find("[data-post-score]").text(e.post_score_display),a.find("[data-score-diff]").text(e.score_delta_display).removeClass("score-diff-positive score-diff-negative score-diff-zero").addClass(`score-diff-${e.score_delta_class||"zero"}`),a.find("[data-solve-count]").text(e.solve_count_display).toggleClass("post-revoke-solve-count-high",!!e.high_solve_count);const s=a.find(".post-revoke-account-ban");s.prop("checked",e.calc_banned),s.prop("disabled",!i.canWrite||e.real_banned);const l=a.find(".post-revoke-account-note");l.is(":focus")||l.val(e.note||"")})}function P(t){t.forEach(e=>{const a=o(`[data-challenge-row="${e.challenge_id}"]`);a.length&&(a.find("[data-challenge-pre-score]").text(e.pre_score_display),a.find("[data-challenge-post-score]").text(e.post_score_display),a.find("[data-challenge-score-diff]").text(e.score_delta_display).removeClass("score-diff-positive score-diff-negative score-diff-zero").addClass(`score-diff-${e.score_delta_class||"zero"}`),a.find("[data-challenge-solve-count]").text(e.solve_count_display))})}function h(t){if(t==null||t==="")return"0";const e=Number(t);return Number.isInteger(e)?String(e):e.toFixed(2).replace(/0+$/,"").replace(/\.$/,"")}function S(t){const e=Number(t||0);return Number.isInteger(e)?String(e):e.toFixed(2).replace(/0+$/,"").replace(/\.$/,"")}function _(t,e){const a=t==="solves"?"Solves":"Awards",s=t==="solves"?"Challenge":"Award",l=t==="solves"?"No solves":"No awards";let v="";return e.length?v=e.map(c=>{const R=t==="solves"?c.challenge_name:c.name,T=c.revoked?"checked":"",m=i.canWrite?"":"disabled";return`
          <tr>
            <td class="col-name">${n(R||"")}</td>
            <td class="text-right col-score">${h(c.original_score)}</td>
            <td class="text-right col-score">${h(c.post_score)}</td>
  ...
  `}function L(t){const e=t.challenge;p=null,u=e.challenge_id,o("#post-revoke-detail-modal-title").text(e.name||"Challenge Detail"),o("#post-revoke-detail-panel").html(`
    <div class="d-flex flex-wrap justify-content-between align-items-start">
      <div>
        <div class="text-muted post-revoke-detail-summary">
    ...
    ${B(t.solves||[])}
  `)}function w(t,e=!0){return p=t,u=null,r("Loading detail..."),o("#post-revoke-detail-modal-title").text("Post-Revoke Detail"),o("#post-revoke-detail-panel").html('<div class="text-muted">Loading detail...</div>'),e&&o("#post-revoke-detail-modal").modal("show"),$(g(`/accounts/${t}`),{method:"GET"}).then(a=>{if(!a.success)throw a;I(a.data),r("")}).catch(a=>{r(d(a),!0),f({title:"Error!",body:d(a),button:"Okay"})})}function N(t,e=!0){return p=null,u=t,r("Loading challenge detail..."),o("#post-revoke-detail-modal-title").text("Challenge Detail"),o("#post-revoke-detail-panel").html('<div class="text-muted">Loading detail...</div>'),e&&o("#post-revoke-detail-modal").modal("show"),$(g(`/challenges/${t}`),{method:"GET"}).then(a=>{if(!a.success)throw a;L(a.data),r("")}).catch(a=>{r(d(a),!0),f({title:"Error!",body:d(a),button:"Okay"})})}function A(t){O(t.rows||[]),P(t.challenge_rows||[]),p?w(p,!1):u&&N(u,!1)}function x(t,e){return C(g(`/accounts/${t}`),e).then(A).catch(a=>{r(d(a),!0),f({title:"Error!",body:d(a),button:"Okay"})})}function b(t,e,a){return C(g(`/${t}/${e}`),a).then(A).catch(s=>{r(d(s),!0),f({title:"Error!",body:d(s),button:"Okay"})})}function y(t,e){k[t]&&clearTimeout(k[t]),k[t]=setTimeout(e,600)}function q(t){const e=t==="challenges";o("#post-revoke-scoreboard-view").toggleClass("d-none",e),o("#post-revoke-challenge-view").toggleClass("d-none",!e),o("[data-post-revoke-view-button]").each(function(){const a=o(this),s=a.data("post-revoke-view-button")===t;a.toggleClass("active btn-primary",s).toggleClass("btn-outline-primary",!s).attr("aria-pressed",s?"true":"false")})}o(()=>{o("[data-post-revoke-view-button]").on("click",function(){q(o(this).data("post-revoke-view-button"))}),o("#post-revoke-summary-body").on("click",".post-revoke-detail-button",function(){w(o(this).data("account-id"))}),o("#post-revoke-challenge-body").on("click",".post-revoke-challenge-detail-button",function(){N(o(this).data("challenge-id"))}),o("#post-revoke-summary-body").on("change",".post-revoke-account-ban",function(){const t=o(this);x(t.data("account-id"),{manual_banned:t.prop("checked")})}),o("#post-revoke-summary-body").on("input",".post-revoke-account-note",function(){const t=o(this),e=t.data("account-id");y(`account-${e}`,function(){x(e,{note:t.val()})})}),o("#post-revoke-detail-panel").on("change",".post-revoke-percent",function(){const t=o(this);b(t.data("kind"),t.data("item-id"),{percentage:t.val()})}),o("#post-revoke-detail-panel").on("change",".post-revoke-item-revoke",function(){const t=o(this);b(t.data("kind"),t.data("item-id"),{revoked:t.prop("checked")})}),o("#post-revoke-detail-panel").on("input",".post-revoke-item-note",function(){const t=o(this),e=`${t.data("kind")}-${t.data("item-id")}`;y(e,function(){b(t.data("kind"),t.data("item-id"),{note:t.val()})})}),o("#post-revoke-detail-modal").on("hidden.bs.modal",function(){p=null,u=null,o("#post-revoke-detail-modal-title").text("Post-Revoke Detail"),o("#post-revoke-detail-panel").html('<div class="text-muted">Loading detail...</div>')})});
